...
LCD_NATIVE_SIZE = (240, 320)
LANDSCAPE_ROTATE_DEGREES = int(os.getenv("LCD_2INCH4_LANDSCAPE_ROTATE", "90"))
PORTRAIT_ROTATE_DEGREES = int(os.getenv("LCD_2INCH4_PORTRAIT_ROTATE", "0"))
DISPLAY_ROTATE_DEGREES = int(os.getenv("LCD_2INCH4_DISPLAY_ROTATE", "180"))
...
def prepare_for_lcd_driver(image, orientation):
    # The 2.4-inch vendor examples use the native 240x320 path. Keep using
    # that path and rotate landscape content ourselves for predictable output.
    degrees = LANDSCAPE_ROTATE_DEGREES if orientation == "landscape" else PORTRAIT_ROTATE_DEGREES
    degrees %= 360
    if degrees:
        image = image.rotate(degrees, expand=True)
    if image.size != LCD_NATIVE_SIZE:
        image = image.resize(LCD_NATIVE_SIZE, Image.Resampling.LANCZOS)
    display_degrees = DISPLAY_ROTATE_DEGREES % 360
    if display_degrees:
        image = image.rotate(display_degrees, expand=False)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Mirror the PiCar-X animated face to the 2.4 LCD.")
...
def main():
    ...
    print(
        f"[LCD_2INCH4] orientation={args.orientation} render={target_size[0]}x{target_size[1]} "
        f"driver={LCD_NATIVE_SIZE[0]}x{LCD_NATIVE_SIZE[1]} "
        f"landscape_rotate={LANDSCAPE_ROTATE_DEGREES} display_rotate={DISPLAY_ROTATE_DEGREES}",
        flush=True,
    )
    ...
    def update_lcd():
        try:
            ...
            frame = prepare_for_lcd_driver(render_for_lcd(image, target_size, args.mode), args.orientation)
