        ...
        try:
            ...
            while True:
                ...
                if remaining <= 0:
                    ...
                    return False
                try:
                    event_id = self.sync_events.get(timeout=min(1.0, remaining))
                except queue.Empty:
                    continue
                if event_id == sync_id:
                    self.last_done_at = time.monotonic()
...
class BargeInMonitor:
    def __init__(self, wake_listener, tts: TTSProcess, lcd: LCDMoodDisplay) -> None:
...
def main():
    ...
    barge_in = BargeInMonitor(wake_listener, tts, lcd)
    ...
    try:
        while True:
            if awake:
                ...
                lcd.send_display(text="Now listening...", sleep_fish=False, meteor_shower=False, listening_waves=True)
            ...
            try:
                ...
                if barge_in.interrupted():
                    ...
                    lcd.send_display(text="Now listening...", meteor_shower=False, listening_waves=True)
            ...
            if response_interrupted or barge_in.interrupted():
                response_stream.cancel()
                barge_in.stop()
                print("[BARGE_IN] Response interrupted; returning to listener.", flush=True)
                lcd.send_display(text="Now listening...", meteor_shower=False, listening_waves=True)
                reset_awake_sleep_deadline("after interruption")
                time.sleep(0.05)
                continue
            ...
            if barge_in.interrupted():
                barge_in.stop()
                print("[BARGE_IN] Response playback interrupted; returning to listener.", flush=True)
                lcd.send_display(text="Now listening...", meteor_shower=False, listening_waves=True)
                reset_awake_sleep_deadline("after interruption")
                time.sleep(0.05)
                continue
